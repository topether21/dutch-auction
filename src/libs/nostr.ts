...
const RELAYS = ["wss://relay.deezy.io", "wss://nostr-pub.wellorder.net", "wss://nostr.bitcoiner.social", "wss://relay.damus.io", "wss://nostr.openordex.org"];
...
async function sign(event: Omit<Event, "sig">, privkey: string): Promise<SignedEvent> {
...
function getSellEvent({ inscriptionId, output, networkName = "mainnet", priceInSats, signedPsbt, type = "sell", pubkey }: SellEventParams) {
	const event = {
...
async function signNostrEvent({ inscriptionId, output, priceInSats, signedPsbt, pubkey, privkey }: SellEventParams & { privkey: string }): Promise<SignedEvent> {
...
async function signAndBroadcastEvent({ inscriptionId, output, priceInSats, signedPsbt, pubkey, privkey }: SellEventParams & { privkey: string }) {
	try {
...
const publishEvent = async (event: Event) => {
	...
	try {
		...
		const events: PublishedEvent[] = results.filter((result) => result.status === "fulfilled").map((result) => (result as PromiseFulfilledResult<PublishedEvent>).value);
