...
import { parseEventInput, createErrorResponse, createHttpResponse, } from "@libs/utils";
import inscriptions from "@libs/inscriptions";
import db from "@libs/db";
const eventBridge = new EventBridgeClient({});
export async function createAuction(event) {
    parseEventInput(event);
    const { startTime, decreaseAmount, timeBetweenEachDecrease, startPrice, reservePrice, metadata, nostrAddress, inscriptionId, output, } = event.body;
    const id = v4();
    const auction = {
        id,
        startTime,
    ...
    try {
        ...
        const validStartTime = startTime < now ? now + 5 : startTime;
...
//# sourceMappingURL=create-auction.js.map